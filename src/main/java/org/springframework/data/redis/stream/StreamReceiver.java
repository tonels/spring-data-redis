...
public interface StreamReceiver<K, V extends Record<K, ?>> {
	...
	class StreamReceiverOptions<K, V extends Record<K, ?>> {
		...
		public static <T> StreamReceiverOptionsBuilder<String, ObjectRecord<String, T>> builder(
			...
			SerializationPair<String> serializer = SerializationPair.fromSerializer(StringRedisSerializer.UTF_8);
			SerializationPair<ByteBuffer> body = SerializationPair.raw();
			return new StreamReceiverOptionsBuilder<>().keySerializer(serializer).hashKeySerializer(body)
					.hashValueSerializer(body).objectMapper(hashMapper);
		}
	...
	class StreamReceiverOptionsBuilder<K, V extends Record<K, ?>> {
		...
		/**
		 * Configure a hash body type. Changes the emitted {@link Record} type to {@link ObjectRecord}.
		 *
		 * @param pair must not be {@literal null}.
		 * @return {@code this} {@link StreamReceiverOptionsBuilder}.
		 */
		@SuppressWarnings("unchecked")
		public <NV> StreamReceiverOptionsBuilder<K, ObjectRecord<K, NV>> bodyType(Class<NV> targetType) {
		...
		/**
		 * Configure a hash body mapper. Changes the emitted {@link Record} type to {@link ObjectRecord}.
		 *
